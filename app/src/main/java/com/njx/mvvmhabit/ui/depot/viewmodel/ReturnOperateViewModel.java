...
public class ReturnOperateViewModel extends ToolbarViewModel {
    ...
    public class UIChangeObsevable {
        public SingleLiveEvent<List<ReturnPartRecordEntity>> listChangeEvent = new SingleLiveEvent<>();
    ...
    public void queryRecordList() {
        apiService.queryReturnRecordList(orderTxt.get())
                ...
                .subscribe(new Consumer<BaseResponse<ReturnDataEntity>>() {
                    @Override
                    public void accept(BaseResponse<ReturnDataEntity> response) throws Exception {
                        //请求成功
    ...
    public void uploadRecord() {
        apiService.returnScan(orderTxt.get(), rollTxt.get())
                .compose(RxUtils.<BaseResponse<UserEntity>>bindToLifecycle(getLifecycleProvider()))
                .compose(RxUtils.schedulersTransformer())
                .compose(RxUtils.exceptionTransformer())
                .doOnSubscribe(new Consumer<Disposable>() {
